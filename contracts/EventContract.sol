...
constructor (address _factoryAddress, address payable _organiser, uint m_totalSupply, uint _priceInEth, string memory _name, string memory _symbol,uint _eventDuration) ERC721(_name, _symbol) {
    this_event = Event (
        _factoryAddress,
        _organiser,
        _symbol,
...
function mintToWinner (uint winningTokenId) external {
    uint256 newItemId = tokenIds.current();
    ...
    require (msg.sender == this_event.factoryAddress, " Only the event organiser can invoke this function THROUGH THE FACTORY ADDRESS and can distribute the winning ticket to the winner.");
